  ...
  "parsers": [
    ...
    },
    {
      ...
      "compatible": [
        ...
        self.assertEqual(jello.cli.main(data=self.jc_a_output), self.expected)

    def test_jc_a_parsers(self):
        """
        Test jc -a | jello 'r = _["parsers"]'
        """
        self.expected = '''\
[
  {
  ...
  },
  {
    ...
    "compatible": [
      ...
      "freebsd"
        ...
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"]'), self.expected)

    def test_jc_a_c_parsers(self):
        """
        Test jc -a | jello -c 'r = _["parsers"]'
        """
        ...
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"]', compact=True), self.expected)

    def test_jc_a_l_parsers(self):
        """
        Test jc -a | jello -l 'r = _["parsers"]'
        """
        self.expected = '''\
...
{"name": "yaml", "argument": "--yaml", "version": "1.0", "description": "YAML file parser", "author": "Kelly Brazil", "author_email": "kellyjonbrazil@gmail.com", "details": "Using the ruamel.yaml library at https://pypi.org/project/ruamel.yaml", "compatible": ["linux", "darwin", "cygwin", "win32", "aix", "freebsd"]}'''
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"]', lines=True), self.expected)

    def test_jc_a_parsers_18(self):
        """
        Test jc -a | jello 'r = _["parsers"][18]'
        """
        self.expected = '''\
{
  "name": "id",
  ...
  "magic_commands": [
    "id"
        ...
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]'), self.expected)
    ...
    def test_jc_a_parsers_18_name(self):
        ...
        """
        self.expected = '"id"'
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["name"]'), self.expected)

    def test_jc_a_l_parsers_18_name(self):
        """
        Test jc -a | jello -l 'r = _["parsers"][18]["name"]'
        """
        self.expected = '"id"'
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["name"]', lines=True), self.expected)

    def test_jc_a_r_parsers_18_name(self):
        """
        Test jc -a | jello -r 'r = _["parsers"][18]["name"]'
        """
        self.expected = 'id'
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["name"]', raw=True), self.expected)

    def test_jc_a_parsers_18_compatible(self):
        """
        Test jc -a | jello 'r = _["parsers"][18]["compatible"]'
        """
        self.expected = '''\
[
  "linux",
  ...
  "freebsd"
        ...
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["compatible"]'), self.expected)

    def test_jc_a_c_parsers_18_compatible(self):
        """
        Test jc -a | jello -c 'r = _["parsers"][18]["compatible"]'
        """
        self.expected = '["linux", "darwin", "aix", "freebsd"]'
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["compatible"]', compact=True), self.expected)

    def test_jc_a_l_parsers_18_compatible(self):
        """
        Test jc -a | jello -l 'r = _["parsers"][18]["compatible"]'
        """
        self.expected = '''\
"linux"
"darwin"
"aix"
"freebsd"'''
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["compatible"]', lines=True), self.expected)

    def test_jc_a_lr_parsers_18_compatible(self):
        """
        Test jc -a | jello -lr 'r = _["parsers"][18]["compatible"]'
        """
        self.expected = '''\
linux
darwin
aix
freebsd'''
        self.assertEqual(jello.cli.main(data=self.jc_a_output, query='r = _["parsers"][18]["compatible"]', lines=True, raw=True), self.expected)

    def test_jc_a_c_list_comprehension(self):
        """
        Test jc -a | jello -c 'r = [entry["name"] for entry in _["parsers"] if "darwin" in entry["compatible"]]'
        """
